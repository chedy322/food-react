...
function Cuisine() {
  ...
  const appel=async (name)=>{
    try{
      let response=await fetch(`https://api.spoonacular.com/recipes/random?apiKey=28f2fb3ec5e14d2cbf98e1af3b63603e&number=10&tags=vegetarian&name=${name}`)
  ...
  useEffect(()=>{
    appel(params.cuisine)
    console.log(params.cuisine)
  },[params.cuisine])
  return (
    <div>
      <Title>
        La cuisine est : {params.cuisine}
      </Title>
      <Grid>
        {
          cuisine.map((recette)=>{
